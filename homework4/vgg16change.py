...
class VGG16(Model):
    def __init__(self):
        ...
        self.c1 = Conv2D(input_shape=(32, 32, 3),filters=64, kernel_size=(3, 3), padding='same',strides=1)  # 卷积层
        self.b1 = BatchNormalization()
        self.a1 = Activation('relu')
        self.c2 = Conv2D(input_shape=(32, 32, 3), filters=64, kernel_size=(3, 3), padding='same', strides=1)  # 卷积层
        ...
        self.c4 = Conv2D(filters=128, kernel_size=(3, 3), padding='same', strides=1)  # 卷积层
...
#取代原来的for循环
cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_save_path,#存的路径
                                                 save_weights_only=True,#只存权重吗
                                                 # monitor='loss',
                                                 save_best_only=True,#只存monitor指标最优的
                                                 verbose=2)#
history = model.fit(image_gen_train.flow(x_train, y_train, batch_size=128), epochs=30,
                    validation_data=(x_test, y_test),
                    validation_freq=1, callbacks=[cp_callback], verbose=1)#verbose 是否每行都显示
